...
parser.add_argument('--inter_outputs', type=bool, default=True, help='collect inter outputs or not')
parser.add_argument('--loss', type=bool, default=True, help='collect  loss signal or not')
parser.add_argument('--gradients', type=bool, default=True, help='collect gradients or not')
parser.add_argument('--Labels', type=bool, default=True, help='collect labels or not')


exp_parameters = parser.parse_args()
if exp_parameters.loss and exp_parameters.Labels:
  path_extra = "_loss_label_G6"
else:
  path_extra = ""
...
if exp_parameters.inter_outputs:
  with torch.no_grad():
      output_size_list = []
      ...
      fwrList0_ = []  # 长度是layer_num, 存储每一层的输出, 测试集
      ...
      fwrList1_ = []  # 长度是layer_num, 存储每一层的输出, 训练集
...
if exp_parameters.gradients:
  ...
  for j in compute_grad:
      ...
      gradients0.append(torch.zeros(images0.shape[0], *grad_[j].shape))
      gradients1.append(torch.zeros(images1.shape[0], *grad_[j].shape))
  ...
  for j in tq(range(images1.shape[0])):
      ...
      aux_grad_list = []
      for i in range(len(grad_)):
          aux_grad_list.append(torch.flatten(grad_[i]).detach())  # 梯度值拿出来，确保不再参与梯度传播
      grad_ = torch.cat(aux_grad_list, 0).detach()
  ...
  # Gradients and additional info for Nasr attacker (wrt model parameters)
...
df_scores0 = pd.DataFrame(scoreLists0, columns=['Grad wrt model parameters L1',
...
df_scores1 = pd.DataFrame(scoreLists1, columns=['Grad wrt model parameters L1',
                                                'Grad wrt model parameters L2',
                                                'Grad wrt model parameters Linf',
                                                'Grad wrt model parameters Mean',
                                                'Grad wrt model parameters Skewness',
...
df_scores0.to_csv(results_dir / f'scores0_{model_type}_partial{path_extra}.csv', index=False)
df_scores1.to_csv(results_dir / f'scores1_{model_type}_partial{path_extra}.csv', index=False)

np.savez_compressed(results_dir / f'NasrTrain0_{model_type}{path_extra}.npz', *TrainData4Nasr0)
np.savez_compressed(results_dir / f'NasrTrain1_{model_type}{path_extra}.npz', *TrainData4Nasr1)
np.savez_compressed(results_dir / f'NasrAddInfo_{model_type}{path_extra}.npz', *AddInfoAttacker)
